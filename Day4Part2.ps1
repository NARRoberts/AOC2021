...
$winningvalue = 0
$inactiveboards = New-Object System.Collections.Generic.HashSet[int]
$ibl = [System.Collections.ArrayList]::new()
foreach ($num in $numbers)
{
    for($board = 0; $board -lt $boards.Count; $board++)
    {
        if (-not $inactiveboards.Contains($board))
        {
            Mark-Board $boards[$board] $num |out-null
            $res = Check-Board $boards[$board]
            if ($res)
            {
                $inactiveboards.add($board) | Out-Null
                $ibl.Add($board) | out-null
                $winningvalue = Get-BoardValue $boards[$board]
                $winningvalue *= $num
            }
        }
    }
    if ($inactiveboards.Count -eq $boards.Count)
    {
        break
    }
}
